/*******************************************************************************
 * 
 * ActiveWidgets Grid 1.0.1 (GPL). Copyright (C) 2003-2005 ActiveWidgets Ltd.
 * All Rights Reserved. https://sslvpn.tsinghua.edu.cn/,DanaInfo=www.activewidgets.com+
 * 
 * WARNING: This copy is made available to you under the terms of the GNU
 * General Public License and is not suitable for inclusion into commercial or
...
(function() {
	...
	//capture the onmousewheel event of Grid (3rd trivial modification)
	var capture = ["click", "mousedown", "mouseup", "mousemove", "mouseover", "mouseout", "mousewheel"];
...
(function() {
	...
	element.selectNodes = function(path) {
		var result = xpath
				.evaluate(path, this, this.ownerDocument._ns, 7, null);
		var i, nodes = [];
		for (i = 0; i < result.snapshotLength; i++) {
			nodes[i] = result.snapshotItem(i)
		}
...
Active.System.Model.create = function() {
	...
	var join = function() {
		...
		for (i = 1; i < arguments.length; i++) {
			s += arguments[i].substr(0, 1).toUpperCase()
...
Active.System.HTML.create = function() {
	...
	obj.setClass = function(name, value) {
		...
		if (element) {
			...
			element.className = element.className.replace(new RegExp("(active-"
					+ name + "-\\w+|$)"), " active-" + name + "-" + v + " ");
			if (this.$index !== "") {
				return
			}
		}
	...
	function getSpecialParamStr(i) {
		return (specialParams[i] = new RegExp("[\\w\\x2D]*=?:?\\x22?\\{#" + i
				+ "\\}[;\\x22]?"))
	}
...
Active.System.Control.create = function() {
	...
	//enable the oncontextmenu and onselectstart event of Grid (1st trivial modification)
...
Active.Formats.Number.create = function() {
	...
	obj.setTextFormat = function(format) {
		...
		this.r4 = rs + "$1" + rg + "$2" + rg + "$3" + rg + "$4" + rd + "$5"
				+ re
	};
	obj.setTextFormat("#.##")
};
...
Active.Formats.Date.create = function() {
	...
	obj.date = new Date();
	obj.digits = [];
	obj.shortMonths = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug",
			"Sep", "Oct", "Nov", "Dec"];
	obj.longMonths = ["January", "February", "March", "April", "May", "June",
			"July", "August", "September", "October", "November", "December"];
	obj.shortWeekdays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
	obj.longWeekdays = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday",
			"Friday", "Saturday"];
	for (var i = 0; i < 100; i++) {
	...
	obj.setDataTimezone = function(value) {
		if (!value) {
			...
			this._dataTimezoneCode = " GMT" + (value > 0 ? "+" : "-")
...
Active.Templates.Header.create = function() {
	...
	obj.setClass("sort", function() {
		return this.getSortProperty("index") != this.$index ? "none" : this
				.getSortProperty("direction")
	});
	...
	//disable the oncontextmenu and onselectstart event of Grid Header (2nd trivial modification)
	obj.setEvent("oncontextmenu", "return false");
	obj.setEvent("onselectstart", "return false");	
};
	...
	var adjustSize = function() {
		...
		if (data) {
			if (data.scrollHeight) {
				space.runtimeStyle.height = data.scrollHeight > data.offsetHeight
						? data.scrollHeight
						: 0;
				space.runtimeStyle.width = data.scrollWidth > data.offsetWidth
						? data.scrollWidth
						: 0;
...
Active.Controls.Grid.create = function() {
	...
	obj.defineDataProperty("value", function(i, j) {
		...
		var value = Number(text.replace(/[,%\$]/gi, "").replace(/\((.*)\)/,
				"-$1"));
	...
	var startColumnResize = function(header) {
		...
		var endResize = function() {
			...
			var i, selector = "#" + grid.getId() + " .active-column-"
					+ header.getItemProperty("index");
	...
	obj.setSelectionIndex = function(index) {
		...
		try {
			...
			if (data.element().offsetHeight + data.element().scrollTop < row
					.element().offsetTop
					+ row.element().offsetHeight) {
				top = row.element().offsetTop + row.element().offsetHeight
	...
	obj.setAction("columnSort", function(src) {
		...
		var d = (this.getSortProperty("index") == i)
				&& (this.getSortProperty("direction") == "ascending")
				? "descending"
				: "ascending";
		window.status = "Sorting...";
		this.sort(i, d);
...
Active.HTTP.Request.create = function() {
	...
	obj.request = function() {
		...
		this._http = window.ActiveXObject
				? new ActiveXObject("MSXML2.XMLHTTP")
				: new XMLHttpRequest;
		this._http.open(this._requestMethod, URL, this._async, this._username,
				this._password);
		...
		function returnResult() {
			if (self._http.responseXML
					&& self._http.responseXML.hasChildNodes()) {
...
Active.XML.Table.create = function() {
	...
	obj.setXML = function(xml) {
		...
		this._items = this._data
				? this._data.selectNodes(this._itemPath)
				: null;
		this._ready = true
	};
	obj.getXML = function() {
...
/*
 * fix for Firefox 1.0.5 dispatchEvent() regression
 */
